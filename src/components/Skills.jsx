...
export default function Skills() {
  ...
  return (
    <section id="competences" style={{ padding: pad, borderTop: '1px solid rgba(201,169,110,.12)' }}>
        ...
        <div style={{
          ...
          {categories.map((cat, i) => (
            <div key={i} style={{
              ...
              borderLeft: i % (isMobile ? 2 : 4) === 0 ? '1px solid rgba(201,169,110,.15)' : 'none',
              borderTop: isMobile && i >= 2 ? 'none' : '1px solid rgba(201,169,110,.15)',
            }}>
              <div style={{ fontSize: '.65rem', letterSpacing: '.25em', textTransform: 'uppercase',
                color: '#c9a96e', marginBottom: '20px' }}>{cat.title}</div>
              <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {cat.items.map((item, j) => (
                  <li key={j} style={{ display: 'flex', alignItems: 'center', gap: '10px',
                    fontSize: isMobile ? '.78rem' : '.85rem', color: '#6b6657' }}>
